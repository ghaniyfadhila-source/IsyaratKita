...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({
  ...
  return (
    <div id="translation-history-section" className="liquid-glass rounded-3xl p-5 border border-slate-200/80 shadow-xs space-y-4">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        ...
        {history.length > 0 && (
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1 text-xs text-rose-600 hover:bg-rose-50 rounded-xl border border-rose-200 transition-colors"
        ...
        <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
          {history.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="p-3 rounded-2xl bg-white/80 border border-slate-200/80 hover:border-blue-300 transition-all space-y-1.5 group text-xs shadow-2xs"
              >
                ...
                <div className="flex items-center gap-2 text-slate-800">
                  ...
                  <span className="font-bold text-slate-900">{item.translatedResult}</span>
                </div>
              </div>
            );
          })}
